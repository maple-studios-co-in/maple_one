...
export const metadata: Metadata = { title: "Orders · MapleOne" };

export default async function RootLayout({ children }: { children: React.ReactNode }) {
  const user = await getSession();
  const brand = await getBrand();
  if (!user) redirect(adminUrl("/login"));
  const toolOn = await isEnabled(`tool.orders`);
  return (
    <html lang="en" className={`${outfit.variable} ${instrument.variable} h-full`} suppressHydrationWarning>
      <body className="min-h-full">
        {toolOn ? (<SuiteShell user={user} brand={brand} current="orders">{children}</SuiteShell>) : (<ToolDisabled label="Orders" />)}
